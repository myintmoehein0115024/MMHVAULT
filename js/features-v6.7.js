/* MMHVAULT FEATURES v6.7 — reliable textured WebGL Earth + JS-driven orbital motion */
(()=>{
  ...
  let webglReady=false, angle=0, paused=false, last=0;
    ...
    if(sv&&sf){
      ...
      if(gl.getProgramParameter(pr,gl.LINK_STATUS)){
        ...
        const drawOrbit=(el,base,speed,dir)=>{
          ...
          const ang=base.z+angle*speed*dir;
          el.style.transform=`translate(-50%,-50%) rotateX(${x}deg) rotateY(${y}deg) rotateZ(${ang}deg)`;
        };

        const frame=ms=>{
          const dt=Math.min(40,Math.max(0,ms-last||16)); last=ms;
          if(!paused){
            if(motionOK) angle+=(dt*.001);
            ...
            if(textureReady){
              ...
              gl.uniformMatrix4fv(um,false,mat4Mul(ry((motionOK?angle*.075:0)+curX*.16),rx(curY*.11)));
              gl.uniform1f(ur,motionOK?angle*.0105:0);
              gl.activeTexture(gl.TEXTURE0);gl.bindTexture(gl.TEXTURE_2D,tx);
              gl.drawElements(gl.TRIANGLES,I.length,indexType,0);
            }
            if(motionOK){
              drawOrbit(orbitEls[0],{x:68,y:0,z:-8},.62,1);
              drawOrbit(orbitEls[1],{x:62,y:7,z:16},.50,-1);
              drawOrbit(orbitEls[2],{x:0,y:66,z:-10},.38,1);
            }
          }
