    ...
    if(sv&&sf){
      ...
      if(gl.getProgramParameter(pr,gl.LINK_STATUS)){
        ...
        const frame=ms=>{
          ...
          if(!paused){
            if(motionOK){ earthAngle += dt*0.00012; orbitAngle += dt*0.00070; }
            ...
            if(motionOK){
              ...
              drawOrbit(orbitEls[0],{x:68,y:0,z:-8},.90,1);
              drawOrbit(orbitEls[1],{x:62,y:7,z:16},.68,-1);
              drawOrbit(orbitEls[2],{x:0,y:66,z:-10},.48,1);
            }else{
              flowDots.forEach((dot,i)=>{
