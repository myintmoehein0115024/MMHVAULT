/* MMHVAULT FEATURES v7.0 — planetary Earth + local orbital satellites */
(()=>{
  ...
  host.classList.add('arch-v71-live');
  ...
  const satSpecs=[
    {orbit:orbitEls[0],cls:'arch65-satellite arch65-sat-cyan',phase:.15,speed:.64},
    {orbit:orbitEls[0],cls:'arch65-satellite arch65-sat-cyan',phase:3.35,speed:.64},
    {orbit:orbitEls[1],cls:'arch65-satellite arch65-sat-gold',phase:1.2,speed:-.48},
    {orbit:orbitEls[1],cls:'arch65-satellite arch65-sat-gold',phase:4.55,speed:-.48},
    {orbit:orbitEls[2],cls:'arch65-satellite arch65-sat-cyan',phase:2.2,speed:.39}
  ];
  const satellites=satSpecs.map(s=>{const el=document.createElement('span');el.className=s.cls;s.orbit.appendChild(el);return {...s,el};});

  // Duplicate each orbital plane as a clipped foreground half so the orbit visually
  // travels behind the Earth and then reappears in front, like a satellite path.
  const frontOrbits=orbitEls.map(el=>{
  ...
  [...orbitEls,...frontOrbits].forEach(el=>el.style.setProperty('--v7-no-pseudo','1'));
  const frontByBase=new Map(orbitEls.map((el,i)=>[el,frontOrbits[i]]));
  host.querySelectorAll('.arch65-flow-dot').forEach(el=>{el.style.display='none';});
    ...
    if(sv&&sf){
      ...
      if(gl.getProgramParameter(pr,gl.LINK_STATUS)){
        ...
        const drawOrbit=(el,base,speed,dir)=>{
          ...
          const transform=`translate3d(-50%,-50%,0) rotateX(${x}deg) rotateY(${y}deg) rotateZ(${ang}deg)`;
          el.style.transform=transform;
          const front=frontByBase.get(el);
        ...
        const frame=ms=>{
          ...
          if(!paused){
            if(motionOK){ earthAngle += dt*0.00062; orbitAngle += dt*0.00128; }
            curX+=(targetX-curX)*.045;curY+=(targetY-curY)*.045;
            if(textureReady){
            ...
            if(motionOK){
              ...
              satellites.forEach(s=>{
                const rr=s.orbit.getBoundingClientRect();
                ...
                s.el.style.transform=`scale(${(0.82+0.22*(0.5+0.5*Math.sin(a*1.6))).toFixed(2)})`;
              });
              ...
              drawOrbit(orbitEls[0],{x:72,y:12,z:-26},.42,1);
              drawOrbit(orbitEls[1],{x:68,y:-14,z:20},.34,-1);
              drawOrbit(orbitEls[2],{x:12,y:72,z:-16},.26,1);
            }else{
              flowDots.forEach((dot,i)=>{
